...
namespace PartyGDK.Base.Miscellaneous
{
    ...
    public class AudioChannel
    {
        ...
        /// </summary>
        public void SetVolume(float? volume = null, bool multiplyByFade = true)
        {
            if (volume != null)
                AudioSource.volume = (float)volume * (multiplyByFade ? FadeMultiplier : 1f);
            else
                AudioSource.volume = Volume * (multiplyByFade ? FadeMultiplier : 1f);
        }
    ...
    public class AudioManager : MonoBehaviour
    {
        ...
        {
            StartCoroutine(LerpFadeMultiplier(channel, true, duration));
        }
        ...
        {
            StartCoroutine(LerpFadeMultiplier(channel, false, duration, stop ? (() => channel.AudioSource.Stop()) : null));
        }

        private IEnumerator LerpFadeMultiplier(AudioChannel channel, bool increase, float duration, Action onFinish = null)
        {
            float lerpTime = 0;

            float start = increase ? 0f : 1f;
            float end = increase ? 1f : 0f;

            while (lerpTime < 1)
            {
                lerpTime = Mathf.Clamp01(lerpTime + Time.deltaTime / duration);
                channel.FadeMultiplier = Mathf.Lerp(start, end, lerpTime);
                channel.SetVolume();
