...
function getRoom() {
    ...
    let isAudience = !moderator && $('#joinAs').val() === 'audience';
    ...
    $.ajax({
        ...
        success: res => {
            ...
            if (moderator) {
                ...
                if (!(res.body.audienceEnabled)) {
                    enableButton = false;
                    buttonText = 'Зрители отключены';
                } else {
                    buttonText = 'Войти';
                }
            } else {
...
function play(enteredPassword) {
    ...
    document.title = gameData.roomData.appName;
}
...
function onWsOpen() {
    ...
    if (pingInterval > 0)
        gameData.pingTask = setInterval(ping, pingInterval * 1000);
}
...
function onWsClose() {
    if (gameData.pingTask) {
        clearInterval(gameData.pingTask);
    }
...
$(document).ready(() => {
    let roomCode = getUrlParameter('code') || getCookie('roomCode');
